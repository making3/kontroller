...
FlowControl.prototype.perform = function perform(actionName, req, res, next) {
   if (this.occupied) {
        return setImmediate(() => {
            this.perform.call(this, actionName, req, res, next);
        });
    }
            ...
            try {
                ...
                if (res && 'function' === typeof res.then) {
                    res.then(function(data) {
                        if (ctl.req.res._headerSent || ctl.send !== 'function') {
                            return ctl.next();
                        }
                        if (ctl.req.query.format === 'html' || ctl.req.body.format === 'html') {
                            if (data && 'object' === typeof data) {
                                Object.keys(data).forEach(function(key) {
                                    ctl.locals[key] = data[key];
                                });
                            }
                            ctl.render();
                        } else {
